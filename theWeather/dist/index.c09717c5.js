...
var _storageMjs = require("./storage.mjs");
...
_storageMjs.storage.createCitiesListAndDefaultCity();
function getCelsius(temperature) {
    return (temperature - 273).toFixed(0);
}
function DeleteActiveClassesTabs() {
    _viewMjs.UI_ELEMENTS.TABS.forEach((tab)=>tab.classList.remove('main-tabs__block--active')
    );
    _viewMjs.UI_ELEMENTS.TABS_BUTTONS.forEach((tabBtn1)=>tabBtn1.classList.remove('main-tabs__item--active')
    );
}
function AddActiveClassesTabs(currentTab, tabBtn2) {
    currentTab.classList.add('main-tabs__block--active');
    tabBtn2.classList.add('main-tabs__item--active');
}
...
function changeCityTitles(json) {
    _viewMjs.UI_ELEMENTS.CITY_TITLES.forEach((cityTitle)=>{
...
function changeWeatherInformation(json) {
    json.then((result)=>{
        ...
        _viewMjs.UI_ELEMENTS.TEMPERATURE_NOW.textContent = getCelsius(result.main.temp) + '°';
        _viewMjs.UI_ELEMENTS.TEMPERATURE_DETAILS.textContent = `Temperature: ${getCelsius(result.main.temp)}°`;
        _viewMjs.UI_ELEMENTS.TEMPERATURE_DETAILS_FEELS.textContent = `Feels like: ${getCelsius(result.main.feels_like)}°`;
        _viewMjs.UI_ELEMENTS.WEATHER.textContent = `Weather: ${result.weather[0].main}`;
        _viewMjs.UI_ELEMENTS.SUNRISE.textContent = `Sunrise: ${sunriseTime.getHours()}:${sunriseTime.getMinutes()}`;
        _viewMjs.UI_ELEMENTS.SUNSET.textContent = `Sunset: ${sunsetTime.getHours()}:${sunsetTime.getMinutes()}`;
...
function changeForecastInformation(json) {
    ...
    for (let forecastItem of forecastItems)forecastItem.remove();
    json.then((result)=>{
        for (let item of result.list){
            ...
            li.innerHTML = `<div class="weather-forecast__top">
                                        <p class="weather-forecast__text">${_dateFns.format(date, 'd MMMM')}</p>
                                        <p class="weather-forecast__text">${_dateFns.format(date, 'HH:mm')}</p>
                                    </div>
            ...
            _viewMjs.UI_ELEMENTS.FORECAST_BLOCK.append(li);
        }
    });
}
...
function addEventListenersUIElements() {
    ...
    for (let openCityBtn of openCityButtons)openCityBtn.addEventListener('click', ()=>{
        ...
        _storageMjs.storage.changeCurrentCity(cityName);
        _viewMjs.UI_ELEMENTS.HEART_BTN.classList.add('weather-now__btn--active');
    });
}
function checkFavoriteCity(cityName) {
    const getFavoriteCities = _storageMjs.storage.getFavoriteCities();
    if (getFavoriteCities.has(cityName)) _viewMjs.UI_ELEMENTS.HEART_BTN.classList.add('weather-now__btn--active');
    else _viewMjs.UI_ELEMENTS.HEART_BTN.classList.remove('weather-now__btn--active');
}
_viewMjs.UI_ELEMENTS.FORM_SEARCH.addEventListener('submit', ()=>{
    const cityName = _viewMjs.UI_ELEMENTS.INPUT_SEARCH.value;
    const jsonWeather = getResponse(cityName, PAGE_WEATHER);
    const jsonForecast = getResponse(cityName, PAGE_FORECAST);
    changeCityTitles(jsonWeather);
    changeWeatherInformation(jsonWeather);
...
_viewMjs.UI_ELEMENTS.HEART_BTN.addEventListener('click', ()=>{
    const isNotActiveClass = _viewMjs.UI_ELEMENTS.HEART_BTN.className !== 'weather-now__btn weather-now__btn--active';
    const cityName = _viewMjs.UI_ELEMENTS.HEART_BTN.previousElementSibling.textContent;
    if (isNotActiveClass) {
        createCityElement(cityName);
        _storageMjs.storage.saveFavoriteCities(cityName);
        addEventListenersUIElements();
        _viewMjs.UI_ELEMENTS.HEART_BTN.classList.add('weather-now__btn--active');
    }
});
for (let tabBtn of _viewMjs.UI_ELEMENTS.TABS_BUTTONS)tabBtn.addEventListener('click', ()=>{
...
window.onload = ()=>{
    const favoriteCities = _storageMjs.storage.getFavoriteCities();
    const currentCity = _storageMjs.storage.getCurrentCity();
    ...
    favoriteCities.forEach((favoriteCity)=>{
        ...
        addEventListenersUIElements();
        if (favoriteCity === currentCity) _viewMjs.UI_ELEMENTS.HEART_BTN.classList.add('weather-now__btn--active');
    });
    changeCityTitles(jsonWeather);
    changeWeatherInformation(jsonWeather);
    changeForecastInformation(jsonForecast);
...
},{"./view.mjs":"i1yN4","./storage.mjs":"kPWK2","date-fns":"9yHCA"}],"i1yN4":[function(require,module,exports) {
...
const UI_ELEMENTS = {
    ...
    CITY_TITLES: document.querySelectorAll('.weather-title'),
    TEMPERATURE_NOW: document.querySelector('.weather-now__temperature'),
    TEMPERATURE_DETAILS: document.querySelector('.weather-temperature'),
    TEMPERATURE_DETAILS_FEELS: document.querySelector('.weather-temperature__feels'),
    HEART_BTN: document.querySelector('.weather-now__btn'),
    CITY_BLOCK: document.querySelector('.city-list'),
    WEATHER: document.querySelector('.weather'),
    SUNRISE: document.querySelector('.weather-sunrise'),
    SUNSET: document.querySelector('.weather-sunset'),
    FORECAST_BLOCK: document.querySelector('.weather-forecast__list')
};
...
},{}],"kPWK2":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const storage = {
    createCitiesListAndDefaultCity: ()=>{
        ...
        if (_jsCookieDefault.default.get('currentCity') === undefined) _jsCookieDefault.default.set('currentCity', 'Vladivostok');
    },
    ...
    changeCurrentCity: (cityName)=>{
        ...
        _jsCookieDefault.default.set('currentCity', cityName, {
            expires: cookieTime
        });
    },
    getCurrentCity: ()=>{
        return _jsCookieDefault.default.get('currentCity');
    }
};
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["gHmlo","kkGe5"], "kkGe5", "parcelRequire8a03")
